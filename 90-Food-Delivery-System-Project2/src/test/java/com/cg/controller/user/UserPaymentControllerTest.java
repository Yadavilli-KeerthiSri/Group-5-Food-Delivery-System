 ...
 * - Controller-only: all services mocked.
...
class UserPaymentControllerTest {
    ...
    void update_shouldRedirectAndCallRegister() throws Exception {
        mvc.perform(post("/user/payment/update")
                        // minimal form param to bind a CustomerDto (adjust if your controller needs more)
                        .param("customerId", "1"))
    ...
    /* 3) CHECKOUT (POST /user/payment/checkout) -> returns 'user/payment' with 'order' model */
    @Test
    ...
    void checkout_shouldReturnPaymentViewWithOrderModel() throws Exception {
        // If your controller expects to create/fill a PaymentDto or similar, we only assert model presence.
        mvc.perform(post("/user/payment/checkout")
                        .param("totalAmount", "499.99"))
           .andExpect(status().isOk())
           .andExpect(view().name("user/payment"))
           .andExpect(model().attributeExists("order")); // adjust key if your controller uses a different attribute name
    }
