...
class AdminOrderControllerTest {
	...
	@Test
	@DisplayName("POST /admin/orders/update/{id} (success) → redirects with success")
	void updateStatus_success_shouldRedirectWithSuccess() throws Exception {
		...
		mvc.perform(post("/admin/orders/update/7")).andExpect(status().is3xxRedirection())
	...
	// void)
	@Test
	@DisplayName("POST /admin/orders/update/{id} (failure) → redirects with error")
	void updateStatus_failure_shouldRedirectWithError() throws Exception {
		...
		mvc.perform(post("/admin/orders/update/9")).andExpect(status().is3xxRedirection())
