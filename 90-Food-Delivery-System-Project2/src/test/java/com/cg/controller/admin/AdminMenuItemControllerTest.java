...
class AdminMenuItemControllerTest {
    ...
    void list_shouldReturnItemsView() throws Exception {
        ...
        mockMvc.perform(get("/admin/menu-items"))
               ...
               .andExpect(view().name("admin/menu-items"))
    ...
    void addForm_shouldReturnFormWithRestaurants() throws Exception {
        ...
        mockMvc.perform(get("/admin/menu-items/add"))
               .andExpect(status().isOk())
               .andExpect(view().name("admin/menu-item-form"))
    ...
    // 4️⃣ EXCEPTION — Delete throws IllegalStateException → redirect to Referer with flash error
    @Test
    ...
    void delete_whenBusinessRuleViolation_shouldRedirectBackWithFlash() throws Exception {
        ...
        doThrow(new IllegalStateException("Cannot delete this item"))
                .when(menuItemService).delete(anyLong());

        mockMvc.perform(get("/admin/menu-items/delete/5")
                        .header(HttpHeaders.REFERER, "/admin/menu-items"))
               .andExpect(status().is3xxRedirection())
               .andExpect(redirectedUrl("/admin/menu-items"))
               .andExpect(flash().attribute("error", "Cannot delete this item"));
    }

    // 🔁 If you instead want to cover unexpected error on list (optional replacement for #3):
    // @Test
