...
/**
 * Focused web-layer test for AdminCustomerController. Loads Controller + MVC
 * infra, and wires in a mocked ICustomerService. Includes
 * GlobalExceptionHandler to assert exception handling behavior.
 */
...
class AdminCustomerControllerTest {
	...
	void shouldReturnCustomerDetailsView_whenCustomerExists() throws Exception {
		...
		mockMvc.perform(get("/admin/clients/{id}", 42L)).andExpect(status().isOk())
				.andExpect(view().name("admin/customer-details")).andExpect(model().attributeExists("customer"));
	...
	void shouldRedirectWithFlashError_whenServiceThrowsIllegalState() throws Exception {
		...
		when(customerService.getById(anyLong())).thenThrow(new IllegalStateException("Business rule violated"));

		mockMvc.perform(get("/admin/clients/{id}", 5L).header(HttpHeaders.REFERER, referer))
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl(referer))
	...
	void shouldRedirectWithGenericError_whenUnexpectedExceptionOccurs() throws Exception {
		final String referer = "/admin/clients/list";
		when(customerService.getById(anyLong())).thenThrow(new RuntimeException("DB connection lost"));

		mockMvc.perform(get("/admin/clients/{id}", 7L).header(HttpHeaders.REFERER, referer))
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl(referer))
	...
	void shouldRedirectToDefault_whenIllegalStateAndNoReferer() throws Exception {
		...
		when(customerService.getById(10L)).thenThrow(new IllegalStateException("Customer is locked"));

		mockMvc.perform(get("/admin/clients/{id}", 10L)).andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl(defaultRedirect)).andExpect(flash().attribute("error", "Customer is locked"));
	...
	void shouldRedirectToDefaultWithGenericMessage_whenUnexpectedAndNoReferer() throws Exception {
		...
		when(customerService.getById(11L)).thenThrow(new RuntimeException("Any failure"));

		mockMvc.perform(get("/admin/clients/{id}", 11L)).andExpect(status().is3xxRedirection())
	...
	void shouldPopulateModelCustomer() throws Exception {
		...
		mockMvc.perform(get("/admin/clients/{id}", 42L)).andExpect(status().isOk())
				.andExpect(model().attributeExists("customer")).andExpect(content().string(containsString(""))); // may
																													// be
																													// no-op
																													// depending
																													// on
