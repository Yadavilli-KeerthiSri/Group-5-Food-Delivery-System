...
public class DeliveryAgentService implements IDeliveryAgentService {
	...
	public DeliveryAgentDto add(DeliveryAgentDto dto) {
		DeliveryAgent entity = DeliveryAgentMapper.fromCreateDto(dto, ids -> orderRepository.findAllById(ids));
	...
	public List<DeliveryAgentDto> getAll() {
		return deliveryAgentRepository.findAll().stream().map(DeliveryAgentMapper::toDto).collect(Collectors.toList());
	}
