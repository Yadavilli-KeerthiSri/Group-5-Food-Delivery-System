...
class AdminRestaurantControllerTest {
	...
	/* 2) UPDATE */
	@Test
	@DisplayName("PUT /admin/restaurants/update/{id} → redirects to list and calls update()")
	void update_shouldRedirect() throws Exception {
	    mvc.perform(put("/admin/restaurants/update/10") 
	            .flashAttr("restaurant", new RestaurantDto())) // Matches @ModelAttribute("restaurant")
