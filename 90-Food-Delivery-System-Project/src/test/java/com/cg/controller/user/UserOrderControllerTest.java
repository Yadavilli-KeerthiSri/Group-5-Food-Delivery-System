...
/**
 * Very simple tests for UserOrderController: - Controller-only (services
 * mocked). - Security filters disabled to avoid 401/403. - View/redirect
 * assertions with minimal verify().
 */
...
class UserOrderControllerTest {
	...
	void success_shouldReturnPaymentSuccessView() throws Exception {
		mvc.perform(get("/user/orders/success")).andExpect(status().isOk())
	...
	void orderDetails_shouldReturnDetailsView() throws Exception {
		...
		mvc.perform(get("/user/orders/5")).andExpect(status().isOk()).andExpect(view().name("user/order-details"))
	...
	void viewMyOrders_shouldReturnMyOrdersView() throws Exception {
		when(orderService.getOrdersByCustomerEmail("user@example.com")).thenReturn(Collections.emptyList());

		mvc.perform(get("/user/orders/my-orders").principal((Principal) () -> "user@example.com"))
				.andExpect(status().isOk()).andExpect(view().name("user/my-orders"))
