...
public final class DeliveryAgentMapper {
	...
	public static DeliveryAgentDto toDto(DeliveryAgent entity) {
		...
		List<Long> orderIds = entity.getOrders() == null ? null
				: entity.getOrders().stream().filter(Objects::nonNull).map(Order::getOrderId)
	...
	// ===== Create =====
	public static DeliveryAgent fromCreateDto(DeliveryAgentDto dto, Function<List<Long>, List<Order>> orderResolver) {
		if (dto == null)
			return null;
	...
	// ===== Update =====
	public static void applyUpdate(DeliveryAgentDto dto, DeliveryAgent target,
