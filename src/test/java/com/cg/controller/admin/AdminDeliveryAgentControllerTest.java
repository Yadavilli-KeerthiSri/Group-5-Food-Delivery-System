...
class AdminDeliveryAgentControllerTest {
	...
	void testListAgents() throws Exception {
		...
		mockMvc.perform(get("/admin/agents")).andExpect(status().isOk()).andExpect(view().name("admin/agents"))
	...
	void testEditForm() throws Exception {
		...
		mockMvc.perform(get("/admin/agents/edit/10")).andExpect(status().isOk())
				.andExpect(view().name("admin/agent-form")).andExpect(model().attributeExists("agent"));
	}
	...
	void testDelete_BusinessException() throws Exception {
		...
		// ❌ was: when(service.delete(anyLong())).thenThrow(new
		// IllegalStateException("Cannot delete this agent"));
		// ✅ correct for void method:
		doThrow(new IllegalStateException("Cannot delete this agent")).when(service).delete(anyLong());

		mockMvc.perform(delete("/admin/agents/delete/5").header(HttpHeaders.REFERER, "/admin/agents"))
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/admin/agents"))
	...
	void testUnexpectedException() throws Exception {
		...
		mockMvc.perform(get("/admin/agents")).andExpect(status().is3xxRedirection())
