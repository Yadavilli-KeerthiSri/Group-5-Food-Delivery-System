...
public class UserRestaurantController {
	...
	@Autowired
	public UserRestaurantController(IRestaurantService restaurantService, IMenuItemService menuItemService) {
	...
	public String listRestaurants(
			@RequestParam(name = "filter", required = false, defaultValue = "all") String filterType, Model model) {

		List<RestaurantDto> restaurants = "top".equalsIgnoreCase(filterType) ? restaurantService.findTopRated()
	...
	/** DASHBOARD (Brought back from previous version) */
	@GetMapping("/dashboard")
	public String showDashboard(Model model) {
		// Uses the specific dashboard logic (Top 6 with fallback)
		List<RestaurantDto> topRestaurants = restaurantService.findTopForDashboard();
